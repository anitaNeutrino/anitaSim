  ...
  : fSettings(settings)
{
  ...
  LASTTIMETOTESTL1_ANITA3=((double)Anita::HALFNFOUR)*anita1->TIMESTEP-icemc::Tools::max(L1_COINCIDENCE_ANITA3); // can't test L1 after this point because the l1_coincidence windows go past the end of the waveform.
  ...
  LASTTIMETOTESTL1_ANITA4LR_SCA=((double)Anita::HALFNFOUR)*anita1->TIMESTEP-LASTTIMETOTESTL1_ANITA4LR_SCA; // can't test L1 after this point because the l1_coincidence windows go past the end of the waveform.
  ...
  LASTTIMETOTESTL1_ANITA4=((double)Anita::HALFNFOUR)*anita1->TIMESTEP-LASTTIMETOTESTL1_ANITA4;
  ...
  LASTTIMETOTESTL1_ANITA4LR_SCB=((double)Anita::HALFNFOUR)*anita1->TIMESTEP-L1_COINCIDENCE_ANITA4LR_SCB; // can't test L1 after this point because the l1_coincidence windows go past the end of the waveform.;
  LASTTIMETOTESTL2_ANITA4LR_SCB=((double)Anita::HALFNFOUR)*anita1->TIMESTEP-icemc::Tools::max(L2_COINCIDENCE_ANITA4LR_SCB); // can't test L1 after this point because the l1_coincidence windows go past the end of the waveform.;
...
void anitaSim::GlobalTrigger::PassesTrigger(Anita* anita1, int mode, TriggerState& triggerState, double this_threshold) {  
  ...
  else if (fSettings->TRIGGERSCHEME == 3) {

    int inu=0;
    PassesTriggerCoherentSum(anita1, inu, triggerState.passes.data());
     
  }
  else if (fSettings->TRIGGERSCHEME == 4) {

    PassesTriggerSummedPower(anita1);

  }
  else if (fSettings->TRIGGERSCHEME == 5) {
...
void  anitaSim::GlobalTrigger::PassesTriggerBasic(Anita* anita1,
  ...
  for (int ilayer=0;ilayer<fSettings->NLAYERS;ilayer++) {
    for (int k=0;k<Anita::NPOL;k++) {
      // for (int j=0;j<Anita::NPHI_MAX;j++) {
  ...
  for (int ilayer=0;ilayer<fSettings->NLAYERS;ilayer++) {
	...
	for (int iband=0;iband<NBAND;iband++) {
	  ...
	  GetAnitaLayerPhiSector(ilayer,iphi,whichlayer,whichphisector); // Translate Anita physical layer to Anita trigger layer and phi sector (4 layers with 8,8,16,8 phi sector to 3 layers with 16 phi sectors each.  In the nadir layer, 8 of the 16 slots are empty on the trigger layer.)
  ...
  for(int iloc=0;iloc<anita1->NTRIGGERLAYERS;iloc++) { // loop over layers
    ...
    for(int iphi=0;iphi<anita1->PHITRIG[iloc];iphi++) { // loop over phi position
      iphitrig=GetPhiSector(iloc,iphi); // get trigger phi sector
      // counts from 0
	...
	for(int iband=0;iband<NBAND;iband++) {
	  if(channels_compacted_passing[iloc][iphitrig][ipolar][iband] == 1
	    ...
	    if (fSettings->PHIMASKING && fSettings->WHICH==Payload::Anita3){ // only applying channel masking like this if it's Anita-3
  ...
  for(int iloc=0;iloc<anita1->NTRIGGERLAYERS;iloc++){ //This is NLAYERS-1 because NLAYERS counts top 16 antennas as 2 layers
    for (int iphi=0;iphi<anita1->PHITRIG[iloc];iphi++){
      iphitrig=GetPhiSector(iloc,iphi);
      for (int ipolar=0;ipolar<2;ipolar++) {	  
  ...
  if (mode == 1) {// TRIGTYPE=2 -> just require ANTtrig channels pass on 2 antennas
	...
	for (int iphi=0;iphi<16;iphi++) {
      ...
      L2Anita3and4(anita1,vl1trig,vl2trig);

      if (fSettings->PHIMASKING){ // only applying channel masking like this if it's Anita-3
	for (int ipol=0;ipol<2;ipol++) {
	  for (int iphi=0;iphi<16;iphi++) {
	...
	for (int iphi=0;iphi<anita1->PHITRIG[0];iphi++) {
	  for (int ipolar=0;ipolar<anita1->NPOL;ipolar++) {
	    icemc::Tools::reverseTimeOrdering(anita1->HALFNFOUR,anita1->arrayofhits_inanita[ilayer][iphi][ipolar],anita1->arrayofhits_forgaryanderic[ilayer][iphi][ipolar]);
	...
	for (int iphi=0;iphi<anita1->PHITRIG[0];iphi++) {
	  ...
	  icemc::Tools::reverseTimeOrdering(anita1->HALFNFOUR,anita1->l1trig_anita4lr_inanita[ilayer][iphi],anita1->l1trig_anita4lr_forgaryanderic[ilayer][iphi]);
	...
	icemc::Tools::reverseTimeOrdering(anita1->HALFNFOUR,anita1->l2trig_anita4lr_inanita[iphi][1],anita1->l2trig_anita4lr_forgaryanderic[iphi]);
	...
	icemc::Tools::reverseTimeOrdering(anita1->HALFNFOUR,anita1->l3type0trig_anita4lr_inanita[iphi],anita1->l3type0trig_anita4lr_forgaryanderic[iphi]);
	...
	icemc::Tools::reverseTimeOrdering(anita1->HALFNFOUR,anita1->l3trig_anita4lr_inanita[iphi],anita1->l3type1trig_anita4lr_forgaryanderic[iphi]);
      }
	...
	// of the neighboring antennas
      if (fSettings->DISCONES==2) {
	FillInNadir(anita1,triggerState.L2.at(0).at(2));
	FillInNadir(anita1,triggerState.L2.at(1).at(2));
      }
...
// This is the case for (fSettings->TRIGGERSCHEME == 3), that coherent waveform sum is being used to trigger, based on Andres' idea. This is specific to ANITA III's antenna geometry.
  ...
  for (int center_phi_sector_offset = -1; center_phi_sector_offset <= 1; center_phi_sector_offset++){
    ...
    for (unsigned center_phi_sector_index = 0; center_phi_sector_index < N_PHI_SECTORS; ++center_phi_sector_index) {
	  ...
	  for (unsigned window_index = 0; window_index < power_of_summed_wfm.size() - 32; window_index += 16) {
	    ...
	    if (power >= fSettings->COHERENT_THRESHOLD){
...
void anitaSim::GlobalTrigger::PassesTriggerSummedPower(const Anita* anita1){
  ...
  double		SummedPowerThreshold	= fSettings->COHERENT_THRESHOLD;	// The threshold against which all of the events will be compared.
  ...
  for (unsigned index_window = 0; index_window < 31; ++index_window) {
	...
	double hypoth_theta_rad = (theta_hypotheses_deg[index_theta_hypothesis] + 45.) * icemc::constants::RADDEG;
	double hypoth_phi_rad = theta_zero.Phi() * icemc::constants::RADDEG;
	...
	for (unsigned index_antenna = 0; index_antenna < 3; ++index_antenna) {
	  offset[index_antenna] = dist[index_antenna] / icemc::constants::CLIGHT;
	}
...
void anitaSim::GlobalTrigger::PassesTriggerScheme5(Anita* anita1,double this_threshold, int *thispasses) {
  ...
  minsample=(int)(anita1->maxt_diode/anita1->TIMESTEP)+(Anita::NFOUR/4-(int)(anita1->maxt_diode/anita1->TIMESTEP)); //+(int)(anita1->arrival_times[0][anita1->rx_minarrivaltime]/anita1->TIMESTEP);
maxsample=Anita::HALFNFOUR; //-(int)(anita1->arrival_times[0][anita1->rx_minarrivaltime]/anita1->TIMESTEP);
...
void anitaSim::GlobalTrigger::FillInNadir(const Anita* anita1,int ant) { //overloaded function
  ...
  for (int iphi=0;iphi<anita1->NRX_PHI[2];iphi++) {
    ...
    whichphi = 1 << GetPhiSector(2,iphi); // get trigger phi sector
    if (whichphi & ant) { // if this phi sector is masked
      antarray[GetPhiSector(2,iphi)]=1;
 ...
 *			ability to have multiple indexing schemes for a given collection of objects.
 */
int anitaSim::GlobalTrigger::GetPhiSector(int i,int j) { // given trigger layer and index, get phi sector.
 ...
 *			ability to have multiple indexing schemes for a given collection of objects.
 */
void anitaSim::GlobalTrigger::GetAnitaLayerPhiSector(int i,int j,int &whichlayer,int &whichphisector) {
    
    
  if (fSettings->WHICH==Payload::Anita1 ||
      fSettings->WHICH==Payload::Anita1Simple ||
      fSettings->WHICH==Payload::Anita2) {// If Anita 1 or Anita 2
    if (i==0) {
      whichlayer=0;
  ...
  else if (fSettings->WHICH==Payload::Anita3 ||
	   fSettings->WHICH==Payload::Anita4) { // anita 3 or anita 4
    if (i==0) {
      whichlayer=0;
      whichphisector=2*j;
    }
 ...
 *			declared name. That way when fSettings->WHICH would have a value of 9, it would be
 *			written fSettings->WHICH==ANITA3.
 *
...
void anitaSim::GlobalTrigger::L3Trigger(const Anita* anita1,TriggerState& triggerState){
  ...
  for (int i=0;i<anita1->PHITRIG[0];i++) {
	...
	fSettings->WHICH==Payload::Anita2) { // Anita 1, 2
				
      for (int ipolar=0;ipolar<2;ipolar++) {
	if (((fSettings->DISCONES==2) && (triggerState.location.at(ipolar).at(1).at(i)>0 && triggerState.location.at(ipolar).at(2).at(i)>0)) || // second layer and nadir?
	    ((fSettings->DISCONES==2) && (triggerState.location.at(ipolar).at(0).at(i)>0 && triggerState.location.at(ipolar).at(2).at(i)>0)) || // first layer and nadir?
	    ((fSettings->DISCONES==2) && triggerState.locationNadirOnly.at(ipolar).at(i)>0) || // just nadir - does this mean having just a nadir pass makes the whole event pass?
	    (fSettings->DISCONES==1 && (triggerState.location.at(ipolar).at(1).at(i)>0 && triggerState.disconesPassing>=fSettings->NDISCONES_PASS)) || // second layer and discones
	    (fSettings->DISCONES==1 && (triggerState.location.at(ipolar).at(0).at(i)>0 && triggerState.disconesPassing>=fSettings->NDISCONES_PASS))   // top layer and discones
	    ) {
...
//     double first_trigger_time = icemc::Tools::dMin(arrival_times,(anita1->number_all_antennas));
 ...
 */
void anitaSim::GlobalTrigger::delay_align_antenna_waveforms(const std::vector< std::vector < std::vector <double> > >& waveforms, const std::vector < std::vector <unsigned int> >& delays, std::vector < std::vector <double> >& output){
 ...
 */
void anitaSim::GlobalTrigger::sum_aligned_waveforms(const std::vector < std::vector <double> >& waveforms, std::vector <double>& output){
  output.clear();
 ...
 *
 */
double anitaSim::GlobalTrigger::summed_power_window(const std::vector <double>& waveform, unsigned int start_index, unsigned int length){
  double result = 0;
 ...
 */
double anitaSim::GlobalTrigger::three_bit_round(double input, bool round_zero_up, bool allow_zero) {
  // 
  double result = 0.;
  if (input < -3.5){
    result = -3.5;
 ...
 */
void anitaSim::GlobalTrigger::convert_wfm_to_3_bit(const std::vector <double>& wfm, double rms, std::vector <double>& output){
  output.clear();
...
int anitaSim::GlobalTrigger::findahit(const std::vector<int>& myvector,int first,int last) {
...
int anitaSim::GlobalTrigger::findanl3(int *l3,int NPHISECTORS) {
...
void anitaSim::GlobalTrigger::L1Anita4LR_ScB_OneBin(int IZERO,
...
int anitaSim::GlobalTrigger::L1Anita4_OnePhiSector(int IZERO,std::vector<int> &vl0_realtime_bottom, std::vector<int> &vl0_realtime_middle, std::vector<int> &vl0_realtime_top,
						std::vector<int> &vl1_realtime_bottom, std::vector<int> &vl1_realtime_middle, std::vector<int> &vl1_realtime_top) {
  ...
  if (vl1_realtime_bottom[vl1_realtime_bottom.size()-1]==1 ||
      vl1_realtime_top[vl1_realtime_top.size()-1]==1 ||
      vl1_realtime_middle[vl1_realtime_middle.size()-1]==1)  {
    return 1;
  }
  else {
    return 0;
...
int anitaSim::GlobalTrigger::L1Anita4LR_ScA_TwoPhiSectors(int IZERO,int ipolar,
...
int anitaSim::GlobalTrigger::PartofL1Anita4LR_ScA_TwoPhiSectors(int ilayerreverse,int ipolar,int IZERO,
...
void anitaSim::GlobalTrigger::L2Anita4LR_ScB_OnePhiSector_OneBin(int IZERO,std::vector<int> vl1_bottom, 
...
int anitaSim::GlobalTrigger::L3or30Anita4LR_ScB_TwoPhiSectors_OneBin( int IZERO,
...
void anitaSim::GlobalTrigger::delayL0(std::vector<int> &vl0,double delay) {
