 ...
 */
anitaSim::TTreeInterpolator::TTreeInterpolator(TTree* t, TString xAxisText){
 ...
 */
anitaSim::TTreeInterpolator::~TTreeInterpolator(){
 ...
 */
TGraph* anitaSim::TTreeInterpolator::makeSortedTGraph(TString drawText){
 ...
 * @returns a pointer to the created, sorted TGraph.
 */
TGraph* anitaSim::TTreeInterpolator::makeSortedTGraph(TString drawText, TString cutString){
 ...
 */
TGraph* anitaSim::TTreeInterpolator::makeSortedTGraph(TString drawText, Double_t wrapHigh, Double_t wrapLow){
 ...
 * These values can then be interpolated between correctly.
 */
TGraph* anitaSim::TTreeInterpolator::makeSortedTGraph(TString drawText, TString cutString, Double_t wrapHigh, Double_t wrapLow){
  /*
    This function:
       ...
       Note: This does not add it to the std::map of graphs that this class uses for storage.
             To create a graph and add it to the std::map use anitaSim::TTreeInterpolator::add.
  */
 ...
 */
void anitaSim::TTreeInterpolator::add(TString yAxisText){
  add(yAxisText, "", 0);
}
 ...
 * @param cutString defines the cuts to padd to fTree->Draw();
 */
void anitaSim::TTreeInterpolator::add(TString yAxisText, TString cutString){
 ...
 * @param wrapValue value to unwrap around. e.g. 360 for rotations in degrees.
 */
void anitaSim::TTreeInterpolator::add(TString yAxisText, Double_t wrapHigh, Double_t wrapLow){
 ...
 * @param wrapValue value to unwrap around. e.g. 360 for rotations in degrees.
 */
void anitaSim::TTreeInterpolator::add(TString yAxisText, TString cutString, Double_t wrapHigh, Double_t wrapLow){
 ...
 */
TGraph* anitaSim::TTreeInterpolator::get(TString yAxisText){
  /* Use this to access a graph you've made with the add function */

  if(fStringToGraph.count(yAxisText)==0){
    std::cerr << "Can't find TGraph in TTreeInterpolator created with text " + yAxisText << std::endl;

    // Hack for c++98... in 2014
    throw std::invalid_argument("Can't find TGraph in TTreeInterpolator created with text " + std::string(yAxisText.Data()));
  }
  else{
 ...
 */
Double_t anitaSim::TTreeInterpolator::interp(TString yAxisText, Double_t xAxisValue){
