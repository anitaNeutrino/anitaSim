...
std::ostream& operator<<(std::ostream& os, const anitaSim::FlightPath& fp){
  switch (fp){
  case anitaSim::FlightPath::FixedPosition:
    return os << "FlightPath::FixedPosition";
  case anitaSim::FlightPath::Circle80DegreesSouth:
    return os << "FlightPath::Circle80DegreesSouth";
  case anitaSim::FlightPath::AnitaLite:
    return os << "FlightPath::AnitaLite";
  case anitaSim::FlightPath::Custom:
    return os << "FlightPath::Custom";
  case anitaSim::FlightPath::Anita1:
    return os << "FlightPath::Anita1";
  case anitaSim::FlightPath::Anita2:
    return os << "FlightPath::Anita2";
  case anitaSim::FlightPath::Anita3:
    return os << "FlightPath::Anita3";
  case anitaSim::FlightPath::Anita4:
...
double anitaSim::FlightDataManager::getPitch() const {
  switch(WHICHPATH){
...
double anitaSim::FlightDataManager::getRoll() const {
  switch(WHICHPATH){
  case FlightPath::Anita2:
...
anitaSim::FlightDataManager::~FlightDataManager(){
}
...
// void anitaSim::FlightDataManager::SetDefaultBalloonPosition() { // position of surface of earth under balloon
...
//       altitude_bn=120000*12.*icemc::constants::CMINCH/100.; // 120000 ft.=36.6 m
//     }
//     else{
//       altitude_bn=BN_ALTITUDE*12.*icemc::constants::CMINCH/100.; // converts the altitude in the input file to meters
//     }
...
void anitaSim::FlightDataManager::ReadAnitaliteFlight() {
...
void anitaSim::FlightDataManager::InitializeBalloon(const Settings* settings) {
...
void anitaSim::FlightDataManager::getBalloonPosition(double eventTime, Anita *anita1) {
  ...
  if (WHICHPATH==FlightPath::AnitaLite ||
    ...
    if (WHICHPATH==FlightPath::AnitaLite){
      altitude_bn=altitude*12.*icemc::constants::CMINCH/100.;
    }
    else{
    ...
    if (BN_ALTITUDE!=0){
      altitude_bn=BN_ALTITUDE*12.*icemc::constants::CMINCH/100.; // set the altitude of the balloon to be what you pick.  This isn't in time for CreateHorizons though!
    }

    icemc::report() << icemc::severity::error << FlightPath::Custom << " is currently broken!" << std::endl;
...
Adu5Pat anitaSim::FlightDataManager::pat() const{
  Adu5Pat pat;
