...
anitaSim::Anita::Anita(const Settings* settings, const char* outputDir, const FlightDataManager* bn1){
...
// int anitaSim::Anita::GetRx(int ilayer, int ifold) const { // get antenna number based on which layer and position it is
...
void anitaSim::Anita::GetPayload(const Settings* settings1, const FlightDataManager* bn1){
  // anita-lite payload
...
void anitaSim::Anita::calculate_all_offsets(void) {
  ...
  for (unsigned center_phi_sector_index = 0; center_phi_sector_index < 1; ++center_phi_sector_index) {
	...
	for (unsigned i_layer = 0; i_layer < N_SUMMED_LAYERS; ++i_layer) {
	  for (unsigned i_sector = 0; i_sector < N_SUMMED_PHI_SECTORS; ++i_sector) {
	    hypothesis_offsets[center_phi_sector_index][index_phi][index_theta][i_sector][i_layer] = int(icemc::Tools::round(hypothesis_offset[i_sector][i_layer] / TRIG_TIMESTEP));
...
void anitaSim::Anita::printDifferentOffsets() {
...
void anitaSim::Anita::getDifferentOffsets() {
...
void anitaSim::Anita::calculate_single_offset(const unsigned center_phi_sector_index, const double angle_phi, const double angle_theta, double hypothesis_offset[][3]) {
  ...
  TVector3 normal_vector = TVector3(cos(angle_theta * icemc::constants::RADDEG) * cos((angle_phi+to_center_of_summed_phi_sectors) * icemc::constants::RADDEG), cos(angle_theta * icemc::constants::RADDEG) * sin((angle_phi+to_center_of_summed_phi_sectors) * icemc::constants::RADDEG), sin(angle_theta * icemc::constants::RADDEG));
  ...
  for (signed int phi_sector_offset = phi_start; phi_sector_offset < (int)(phi_start+N_SUMMED_PHI_SECTORS); phi_sector_offset++) {
    ...
    for (unsigned i_layer = 0; i_layer < N_SUMMED_LAYERS; ++i_layer) {
      ...
      double offset = (-1. / icemc::constants::CLIGHT) * normal_vector.Dot(antenna_pos - one_antenna_pos);
...
// void anitaSim::Anita::GetArrivalTimes(const TVector3& rf_direction,Balloon *bn1, const Settings *settings1) {
...
//       arrival_times[ipol][antenna_index] = (antenna_positions[ipol][antenna_index].Dot(rf_direction)) / icemc::constants::CLIGHT;
...
//       double theta_deg =rf_tmp_dir.Theta() * icemc::constants::DEGRAD;//

//       double phi_deg = rf_tmp_dir.Phi() *icemc::constants::DEGRAD;
...
//       theta_offset = 10;//boresight_vector[ant_ctr].Theta()*icemc::constants::DEGRAD;
...
// 	  if(fabs(phi_deg) > fabs(phi_deg+2*icemc::constants::PI)) phi_deg+=2*icemc::constants::PI;
// 	  if(fabs(phi_deg) > fabs(phi_deg-2*icemc::constants::PI)) phi_deg-=2*icemc::constants::PI;
// 	  phi_deg = phi_deg*icemc::constants::DEGRAD;
...
//   //    double last_trigger_time=icemc::Tools::dMax(arrival_times,(number_all_antennas));
...
// void anitaSim::Anita::GetArrivalTimesBoresights(const TVector3 rf_direction[NLAYERS_MAX][NPHI_MAX],Balloon *bn1, const Settings *settings1) {
...
//       arrival_times[ipol][antenna_index] = antenna_positions[ipol][antenna_index].Dot(rf_direction[ilayer][ifold]) / icemc::constants::CLIGHT;
...
// void anitaSim::Anita::GetArrivalTimesBoresights(const TVector3 rf_direction[NLAYERS_MAX][NPHI_MAX]) {
